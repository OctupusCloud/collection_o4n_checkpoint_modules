...
DOCUMENTATION = """
---
module: o4n_cp_delete_https_rules
short_description: Delete https rules of the selected layer.
description:
...
def delete_https_rule(provider, layers, module):
    token = login(provider)
    url_show = "show-https-rule"
    url_set = "delete-https-rule"
    try:
        msg_ret_dict = {}
        output = []
        for desired_layer in layers:
            ...
            for rule in desired_layer["rules"]:
                ...
                if status_show:
                    ...
                    blade = [blade["name"] for blade in response_show["blade"]]
                    action = response_show["action"]["name"]
                    site_category = [site_category["name"] for site_category in response_show["site-category"]]
                    track = response_show["track"]["name"]
                    install_on = [
...
def main():
    ...
    success, msg_ret, output, task_detail, msg_discard, msg_logout = delete_https_rule(
