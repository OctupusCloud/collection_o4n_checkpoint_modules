...
DOCUMENTATION = """
---
module: o4n_cp_add_threat_rules
short_description: Manages threat-rules objects on Check Point over Web Services API
description:
...
def set_threat_rule(provider, token, payload, rule_number):
    url = "set-threat-rule"
...
def add_threat_rule(provider, token, desired_rule, layer):
    url = "add-threat-rule"
    payload = {}
    payload["layer"] = layer
...
def verify_objects_in_rule_and_set(
    ...
    for desired_rule in desired_rules:
        ...
        desired_protected_scope = (
            [desired_rule["protected_scope"]]
            if isinstance(desired_rule["protected_scope"], str)
            else desired_rule["protected_scope"]
        )

        existing_destinations = [
            get_name_from_uid(response, dest) for dest in rule["destination"]
        ]
        existing_sources = [
        ...
        existing_protected_scope = [
            get_name_from_uid(response, protected_scope) for protected_scope in rule["protected-scope"]
        ]

        # Comprobar si todos los destinations, protected_scope, service deseados ya están en la regla
        if (
            ...
            and all(protected_scope in existing_protected_scope for protected_scope in desired_protected_scope)
        ):
            ...
            if missing_sources:
                ...
                validate, rule_set = set_threat_rule(
        ...
        # Comprobar si todos los sources, protected_scope, service deseados ya están en la regla
        elif (
            all(src in existing_sources for src in desired_sources)
            and all(serv in existing_service for serv in desired_service)
            and all(protected_scope in existing_protected_scope for protected_scope in desired_protected_scope)
        ):
            # Comprobar si falta algún destinations deseado en la regla
            ...
            if missing_destinations:
                # Aquí puedes actualizar la regla para añadir los destinations que faltan
                payload_set_rule = {
                    "uid": rule["uid"],
                    "destination": {
        ...
        # Comprobar si todos los sources, destinations, service deseados ya están en la regla
        elif (
            all(src in existing_sources for src in desired_sources)
            and all(serv in existing_service for serv in desired_service)
            and all(dest in existing_destinations for dest in desired_destinations)
        ):
            # Comprobar si falta algún protected_scope deseado en la regla
            missing_protected_scope = [
                dest
                for dest in desired_protected_scope
                if dest not in existing_protected_scope
            ]
            if missing_protected_scope:
                # Aquí puedes actualizar la regla para añadir los protected_scope que faltan
                payload_set_rule = {
                    "uid": rule["uid"],
                    "protected-scope": {
                        "add": missing_protected_scope,
                    },
                    "layer": layer,
                }
...
def add_and_set_threat_rule(provider, layers, module):
    token = login(provider)
    url = "show-threat-rulebase"
    try:
        msg_ret_dict = {}
        output = []
        ...
        for desired_layer in layers:
            ...
            if len(desired_rules) > 0:
                for desired_rule in desired_rules:
                    validate, rule_add = add_threat_rule(
...
def main():
    ...
    (
        ...
        msg_discard,
        msg_logout,
    ) = add_and_set_threat_rule(provider, layers, module)
