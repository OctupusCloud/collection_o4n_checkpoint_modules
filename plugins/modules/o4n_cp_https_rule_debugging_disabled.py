...
DOCUMENTATION = """
---
module: o4n_cp_http_rule_debugging_disabled
...
tasks:
  - name: debug disabled rules
    o4n_cp_http_rule_debugging_disabled:
      layer: "Test Security"
    register: output
"""
RETURN = """
output:
  description: The checkpoint output
  type: dict
  returned: allways
  sample:
    "output": {
...
def delete_https_role(provider, token, response, rule, section, uid, layer):
    url = "delete-https-rule"
    payload = {
        "uid": uid,
        "layer": layer
    }
    ...
    if status:
        if response_delete["message"] == "OK":
            ...
            site_category = [get_name_from_uid(response, vpn) for vpn in rule['site-category']]
            action = get_name_from_uid(response, rule['action'])
            track = get_name_from_uid(response, rule['track'])
            blade = [get_name_from_uid(response, time) for time in rule['blade']]
...
def delete_https_rule_deactivated(provider: dict, layers: list, module: object):
                        ...
                        if rules["type"] == "https-section":
                            ...
                            for rule in rules["rulebase"]:
                                if rule['enabled'] is False:
                                    uid = rule['uid']
                                    status_delete, response_delete = delete_https_role(provider, token, response, rule, section, uid, layer)
                                    if status_delete:
                                        rule_list_deleted.append(response_delete)
                        elif rules["type"] == "https-rule":
                            if rules['enabled'] is False:
                                uid = rules['uid']
                                status_delete, response_delete = delete_https_role(provider, token, response, rules, "N/A", uid, layer)
                                if status_delete:
                                    rule_list_deleted.append(response_delete)
...
def main():
    ...
    (
        ...
        msg_discard,
        msg_logout,
    ) = delete_https_rule_deactivated(provider, layers, module)
