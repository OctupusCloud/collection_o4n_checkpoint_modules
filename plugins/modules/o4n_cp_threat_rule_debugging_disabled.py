...
DOCUMENTATION = """
---
module: o4n_cp_threat_rule_debugging_disabled
...
def delete_threat_rule(provider, token, response, rule, section, uid, layer):
    url = "delete-threat-rule"
    payload = {
        "uid": uid,
        "layer": layer
    }
    ...
    if status:
        if response_delete["message"] == "OK":
            ...
            protected_scope = [get_name_from_uid(response, vpn) for vpn in rule['protected-scope']]
...
def delete_threat_rule_deactivated(provider: dict, layers: list, module: object):
                        ...
                        if rules["type"] == "threat-section":
                            ...
                            for rule in rules["rulebase"]:
                                if rule['enabled'] is False:
                                    uid = rule['uid']
                                    status_delete, response_delete = delete_threat_rule(provider, token, response, rule, section, uid, layer)
                                    if status_delete:
                                        rule_list_deleted.append(response_delete)
                        elif rules["type"] == "threat-rule":
                            if rules['enabled'] is False:
                                uid = rules['uid']
                                status_delete, response_delete = delete_threat_rule(provider, token, response, rules, "N/A", uid, layer)
                                if status_delete:
                                    rule_list_deleted.append(response_delete)
...
def main():
    ...
    (
        ...
        msg_discard,
        msg_logout,
    ) = delete_threat_rule_deactivated(provider, layers, module)
