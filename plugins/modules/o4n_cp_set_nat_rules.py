            ...
            for rule in desired_package["rules"]:
                ...
                if status:
                    rule_add = {
                        ...
                        "original-destination": response['original-destination']["name"],
                        "original-service": response['original-service']["name"],
                        "translated-source": response['translated-source']["name"],
                        "translated-destination": response['translated-destination']["name"],
                        "translated-service": response['translated-service']["name"],
                        "install-on": [install_on["name"] for install_on in response['install-on']]
                    }
...
def main():
                ...
                options = dict (
                    ...
                    rules = dict(
                        ...
                        options = dict (
                            ...
                            rule_number = dict(required=False, type='raw', choice=[str, int, dict]),
                            name = dict(required=False, type='str'),
                            new_position = dict(required=False, type='int'),
                            new_name = dict(required=False, type='str'),
                            enabled = dict(required=False, type='bool'),
                            method = dict(required=False, type='str', choice=["static", "hide", "nat64", "nat46", "cgnat"]),
                            ...
                            install_on = dict(required=False, type='raw', choice=[str, list]),
    ...
    success, msg_ret, output, task_detail, msg_discard, msg_logout = set_nat_rule(provider, packages, module)
    if success:
        module.exit_json(
            failed=False,
            msg=msg_ret,
